...
q=q.substring(1,q.length-1);q!==p.key.toString()&&(analytics.reset(),h=null);if(_.isNull(h)||"null"===h)q={firstName:p.first_name,lastName:p.last_name},p.email&&p.email.address&&(q.email=p.email.address),analytics.identify(p.key,q),h='"'+p.key+'"'}var p=null,r=/^\/course\/.*--.*$/i,s=/^\/course\/viewer/i;q=null;var x=/^\/!c-nd\d{3}.*/i,v=/^\/course\/.*--nd\d{3}$/i;""===n?q=p="Home":"/account"===n.substring(0,8)?"/account/auth/signin"===n?q=p="Sign In":"/account/auth/signup"===n?q=p="Sign Up":(p="My Account Settings",
q="My Account Settings - "+(g(n.substring(9,n.length))||"Contact Info")):"/me"===n.substring(0,3)?q=p="My Home":"/nanodegree"===n?q=p="Nanodegrees":"/courses"===n.substring(0,8)?(p="Catalog",q="Catalog - "+g(n.substring(9,n.length))):"/checkout"===n.substring(0,9)?q=p="Checkout":v.test(n)?(p="Nanodegree Info",q="Nanodegree Info - "+n.split("--")[1]):s.test(n)?(p=/c-([^/]+)/.exec(n),q="unknown",2<=p.length&&(q=p[1]),x.test(n)?(p="My Nanodegree Classroom",q="My Nanodegree Classroom - "+q):(p="My Course Classroom",
q="My Course Classroom - "+q)):r.test(n)&&(p="Course Info",q="Course Info - "+n.split("--")[1]);var m={category:p,path:n,search:m.query,url:d},t={},x=function(a){var b={};b.conceptKey=t.exercise?a.morsel:a.exercise;b.atomKey=a.morsel;b.lessonKey=a.lesson;"nd"==t.course.toLowerCase().substring(0,2)?(b.degreeKey=a.course,b.projectKey=null,b.moduleKey=null):b.courseKey=a.course;return b};if("My Nanodegree Classroom"===p||"My Course Classroom"===p)_.each({course:/c-([^/]+)/,lesson:/l-(\d+)/,morsel:/m-(\d+)/,
exercise:/e-(\d+)/},function(a,b){var d=a.exec(n);d&&d.length&&(t[b]=d[1])}),null==t.morsel?a=!0:_.extend(m,x(t));m.referrer=d===c?document.referrer:c;b.segmentPageProperties={page:m};f.segmentPageProperties={page:m};a||analytics.page(p,q,m)})}]);angular.module("page.account",["ngRoute","core","account"]);
...
if("CHF"===c)return(a/100).toLocaleString("de-CH",{style:"currency",currency:c})+" "+c;if("CNY"===c)return(a/100).toLocaleString("zh-CH",{style:"currency",currency:c})+" "+c;if("BRL"===c)return(a/100).toLocaleString("br-PT",{style:"currency",currency:c})+" "+c;if("SAR"===c)return(a/100).toLocaleString("ar-SA",{style:"currency",currency:c})+" "+c;if("EGP"===c)return(a/100).toLocaleString("ar-EG",{style:"currency",currency:c})+" "+c}else{var b=0>a?"-":"",a=Math.abs(a);if("USD"===c){var d=a%100;return b+
"$"+Math.floor(a/100)+(d?"."+d:"")+" "+c}if("INR"===c)return b+"\u20b9"+Math.floor(a/100)+" "+c;if("GBP"===c){var d=Math.floor(a/100),e=a%100;return b+"\u00a3"+d+(e?"."+e:"")+" "+c}if("EUR"===c)return d=Math.floor(a/100),e=a%100,b+"\u20ac"+d+(e?","+e:"")+" "+c;if("CHF"===c)return d=Math.floor(a/100),e=a%100,b+"CHF "+d+(e?","+e:"");if("CNY"===c)return d=Math.floor(a/100),e=a%100,b+"\u00a5"+d+(e?"."+e:"")+" "+c;if("BRL"===c)return d=Math.floor(a/100),e=a%100,b+"R$"+d+(e?"."+e:"")+" "+c;if("SAR"===c)return d=
Math.floor(a/100),e=a%100,b+d+(e?"."+e:"")+"\ufdfc "+c;if("EGP"===c)return d=Math.floor(a/100),e=a%100,b+"E\u00a3"+d+(e?"."+e:"")+" "+c}return a+c}}]);angular.module("filters").filter("formatCurrency2",[function(){return function(a,c){var b;if("object"==typeof Intl&&Intl&&"function"==typeof Intl.NumberFormat)if("USD"===c)b=(a/100).toLocaleString("en-US",{style:"currency",currency:c});else if("INR"===c)b=(a/100).toLocaleString("en-IN",{style:"currency",currency:c});else if("GBP"===c)b=(a/100).toLocaleString("en-GB",{style:"currency",currency:c});else if("EUR"===c)b=(a/100).toLocaleString("at-DE",{style:"currency",currency:c});else if("CHF"===c)b=
(a/100).toLocaleString("de-CH",{style:"currency",currency:c});else if("CNY"===c)b=(a/100).toLocaleString("zh-CH",{style:"currency",currency:c})+" "+c;else if("BRL"===c)b=(a/100).toLocaleString("br-PT",{style:"currency",currency:c})+" "+c;else{if("SAR"===c)return(a/100).toLocaleString("ar-SA",{style:"currency",currency:c})+" "+c;if("EGP"===c)return(a/100).toLocaleString("ar-EG",{style:"currency",currency:c})+" "+c}else{var d=0>a?"-":"",a=Math.abs(a);if("USD"===c)b=a%100,b=d+"$"+Math.floor(a/100)+(b?
"."+b:"");else if("INR"===c)b=d+"\u20b9"+Math.floor(a/100);else if("GBP"===c){b=Math.floor(a/100);var e=a%100;b=d+"\u00a3"+b+(e?"."+e:"")+" "+c}else if("EUR"===c)b=Math.floor(a/100),e=a%100,b=d+"\u20ac"+b+(e?"."+e:"")+" "+c;else if("CHF"===c)b=Math.floor(a/100),e=a%100,b=d+"CHF "+b+(e?"."+e:"");else{if("CNY"===c)return b=Math.floor(a/100),e=a%100,d+"\u00a5"+b+(e?"."+e:"")+" "+c;if("BRL"===c)return b=Math.floor(a/100),e=a%100,d+"R$"+b+(e?"."+e:"")+" "+c;if("SAR"===c)return b=Math.floor(a/100),e=a%
100,d+b+(e?"."+e:"")+"\ufdfc "+c;if("EGP"===c)return b=Math.floor(a/100),e=a%100,d+"E\u00a3"+b+(e?"."+e:"")+" "+c}}return b?/\.00$/.test(b)?b.slice(0,-3):b:a+c}}]);angular.module("filters").filter("imageCropSize",[function(){return function(a,c){if(a)return a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||a.match(/\/_ah\/img\//)?a=a.replace(/\=s\d+(-c)?/,"=s"+c+"-c"):a.match(/\/\/robohash\.org\//)&&(a=a+"?size="+c+"x"+c),a}}]).filter("imageSize",[function(){return function(a,c,b){if(a){b=0===b?void 0:b;c=0===c?void 0:c;if("number"!==typeof c&&"number"!==typeof b)return a;var d=Math.max(b,c)||b||c;if(a.match(/\/\/[^.\/]+\.(ggpht|googleusercontent)\.com\//)||
...
b){return d.list(a,b).then(function(a){return _.chain(a.degrees).filter(function(a){return 0!==a.key.indexOf("ca")}).map(function(a){if(a.title){var b=a.title.split("Nanodegree")[0];return _.extend(a,{name:b})}return a}).value()})},fetchCatalogAsMapOfNodeKeysToNode:function(a,c){return b=b||d.list(a,c).then(function(a){var b=_.indexBy(a.courses,"key");return b=_.extend(b,_.indexBy(a.degrees,"key"))})}};return d}]);angular.module("gateways").factory("registrar",["api",function(a){var c={_get:function(b,c,e){var c=c||{},f=_.defaults(e||{},{limit:100,offset:0});c.paging=f;return a.get(b,c).then(function(a){var b=null,a=a.data;a.more&&(b={cursor_map:a.cursor_map,offset:f.limit+f.offset,limit:f.limit},delete a.cursor_map,delete a.more,a.nextPage=b);return a})},getSummaries:function(a,d){return c._get("/registrar/"+a+"/summaries",{},d)},queueSummary:function(b,c,e,f,g,h){return a.post("/registrar/"+b+"/summaries?_processing=background",
{lesson_key:e,group_key:c,file_format:f,description:g,perform_regrade:!!h})},getUdacityUserKeys:function(b,c,e){return a.get("/registrar/"+encodeURIComponent(b)+"/search/"+encodeURIComponent(c)+"/"+encodeURIComponent(e)).then(function(a){return a.data})},getAffiliateProfile:function(b,c){return a.get("/registrar/"+encodeURIComponent(b)+"/profiles/"+encodeURIComponent(c)).then(function(a){return a.data.profile})},deleteAffiliateProfile:function(b,c){return a.deleteRequest("/registrar/"+encodeURIComponent(b)+
...
analytics.alias(a.signedInUserAccountKey,analytics.user().anonymousId(),a.segmentPageProperties,function(){n(p)})})})})})},r=function(d){a.errorMessage=b.trustAsHtml("We were unable to validate your account.");return c.reject(d)},s=function(d){if(422===d.data.status)a.errorMessage=b.trustAsHtml("The email address associated with your account has not been verified. Please check your email for further instructions and return here to continue with signing in.");else if(403===d.data.status)a.errorMessage=
b.trustAsHtml('We couldn\'t find a valid account associated with your email. You can sign in with an existing account, or <a href="#!/signup">Sign Up</a> to get started.');else{if(502===d.data.status)return r(d);a.errorMessage=b.trustAsHtml("Something went wrong and we're currently looking into it.")}return c.reject(d)};a.signInEmail=function(){a.buttonDisabled=!0;a.fieldErrors=null;a.email=$("#email").val();a.password=$("#password").val();var d=j.check({email:{pretty:"email address",value:a.email,
...
a.registeredUser&&a.registeredUser.key)analytics.alias(a.registeredUser.key,analytics.user().anonymousId(),a.segmentPageProperties,function(){return i["finally"](function(){return j["finally"](function(){return l["finally"](function(){return k["finally"](d)})})})});else return i["finally"](function(){return j["finally"](function(){return l["finally"](function(){return k["finally"](d)})})})})})},x=function(b){a.isLoading=a.isGoogleBranded?b:!1},v=function(d,e){x(!1);a.errorMessage=b.trustAsHtml("We were unable to validate your "+
d+" account. ");return c.reject(e)},t=function(d,e){x(!1);a.errorMessage=b.trustAsHtml("We were unable to access your "+d+" account.");return c.reject(e)},u=function(d,e,f){x(!1);var h=f.data.status;409===h?("Udacity"===d?a.initialSearch.error_existing_user={message:b.trustAsHtml("This account ("+e+") already exists, please Sign In to continue."),provider:"udacity",email:e}:"Facebook"===d?a.initialSearch.error_existing_user={message:b.trustAsHtml("The account you entered is not valid. Please sign up with a different account to continue."),
...
function(a){return a.plan.id===c.product.key}).id;f=g?d.post("/customers/me",{stripe_token:e.stripe_token}).then(function(){return d.deleteRequest("/customers/me/subscriptions/"+g+"/trial")}):a.reject({data:{}})}else f||(f=d.post("/customers/me/purchases",e));f=f.then(function(a){return a.data.purchase},function(b){c.generalPaymentFormError=b.data.error?x(b.data):"We are having difficulties processing your request. If this error keeps appearing please contact payment-support@udacity.com.";b.data.was_charged&&
...
!1:!a.paymentConfirmed},onSubmit:function(b,d){if(d.customerCard)return t(!1,d);var e=b.creditCard||{},f=c.checkValidity(e);d.creditCardErrors=f.errorText;if(!f.valid)return!1;f=(d.product&&d.product.name)+", "+v(d.product);return c.createPaymentToken(f,e).then(function(a){return t(a,d)},function(b){d.generalPaymentFormError=x(b.error);return a.reject(b)})}},{getLabel:function(a){return a.confirmationStepLabel},view:"/templates/angular/payment/payment_confirmation.html",onStart:function(a,b){b.paymentConfirmed=
...
"FreeProjectTrial","$attrs",function(a,c,b,d,e,f,g,h,i,j,l,k,m,n,p,q,r,s,x,v,t,u,y,M,N,H,I,J,O,K,D,A){function E(){var b;b=_.without(_.union(l.general,l[w]),null,void 0,"");a.viewerLoadingMessage=b[_.random(b.length-1)];a.isViewerLoading=!0}function C(a,b,c){if(a){var d=p[a.model];if(d){if(!c||d.extension)return"/templates/angular/viewer/partials/"+d.prefix+b+"?"+j}else console.error("Unrecognized morsel type "+a.model)}else if(!c)return"/templates/angular/viewer/partials/loading"+b+"?"+j}function F(a){f.cancel(B);
("Video"===a.model||"SketchVideo"===a.model)&&h.addEventListener("videoPlay",function(){f.cancel(B);B=f(G(a.key),1E3*g,!1)},{removeAfterFired:!0})}function G(a){return function(){if(h.getCurrentTime()>=g){var b=h.getCurrentTime();h.emitEvent("videoPause",b);h.emitEvent("videoPlay",b+1);c.markMorselAsCompleted(a)}else B=f(G(a),500,!1)}}a.modalsUrl="/templates/viewer/partials/viewer_modals.html?"+j;a.getCoachServicesUrl=function(){if(a.course)return"/conditional/tutoring?course_id="+window.encodeURIComponent(a.course.key)+
"&cache_bust="+j};a.userIsLoggedIn=t.userIsLoggedIn();a.canEditNotes=y;a.course=null;a.lessons=null;a.lesson=null;a.morsels=null;a.morsel=null;a.tidbit=null;var w=b.path().match("/"+m.course+"([^/]+)"),z=b.path().match("/"+m.lesson+"([^/]+)"),i=b.path().match("/"+m.lesson+"([^/]+)"),w=w?w[1]:null,z=z?z[1]:null,i=i?i[1]:null;a.courseKeyFromUrl=w;a.lessonKeyFromUrl=z;k=["ca001","ca002","ca003","ca801"];A.redirectNewClassroom&&!_.include(k,w)&&(k=w,A=z,/nd.*/i.test(k)?window.location.href="https://classroom.udacity.com/nanodegrees/"+
k+"/syllabus":k?(k="https://classroom.udacity.com/courses/"+k,A&&i&&(k+="/lessons/"+A+"/concepts/"+i+"0923"),window.location.href=k):window.location.href="https://classroom.udacity.com/me");w||(window.location.href="/");s.addEventListener("sessionExpired",function(){window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path())});x&&(window.location.href="/account/auth#!/signin?next=%2Fcourse%2Fviewer%23!"+encodeURIComponent(b.path()));D.fetchFreeTrialComplete(w).then(function(a){a.trialing&&
a.projectComplete&&D.installPaywallModal(a,w,z)});c.addEventListener("lessonPermissionDenied",function(){window.location.href="/course/"+w});c.addEventListener("courseChange",function(a){if((!a.lessons||0===a.lessons.length)&&!a.lastLessonVisited&&!d.lessonKey)window.location.href="/course/"+w}).addEventListener("coursePermissionDenied",function(){d.lessonKey||(window.location.href="/course/"+w)});c.addEventListener("lessonChange",function(a){if(!a.morsels||0===a.morsels.length)window.location.href=
"/course/"+w});a.isViewerLoading=!0;c.addEventListener("courseChange",function(){n.loadLibraries(["codemirror","mathquill"]).then(function(){},function(){I.error("Classroom failed to load a required library");alert("Failed to load some required parts of the course! Please refresh the page. If this issue persists, please contact support@udacity.com. Thanks!")})}).addEventListener("morselChange",function(){a.isViewerLoading=!1});c.addEventListener("courseChange",function(b){a.course=b.course;a.lessons=
b.lessons;a.enrollment=_.find(t.get("_enrollments"),function(a){return a.node_key===b.course.key});a.premiumAccess=u.hasPermissionTo(a.course,"view-paid-supplemental-media")||a.enrollment&&(u.isCurrentEnrolled(a.enrollment,b.course)||u.isGraduated(a.enrollment,b.course));H.set({isPaid:a.premiumAccess?"yes":"no"})},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("lessonChange",function(b){a.lesson=b.lesson;a.morsels=b.morsels},{replayLast:!0,removeOnDestructionOf:a}).addEventListener("morselChange",
function(b){a.morsel=b;F(b)},{replayLast:!0}).addEventListener("tidbitChange",function(b){(a.tidbit=b.tidbit)&&F(b.tidbit)},{replayLast:!0});c.addEventListener("pendingLessonChange",function(){a.morsels=null;a.morsel=null;a.tidbit=null});a.$on("$routeChangeSuccess",function(b,f,h){if(b=!d.morselKey)if(!h||!f)b=!1;else{var g=h.pathParams,f=f.pathParams;if(_.keys(g).length===_.keys(f).length)b=!1;else{var i=!0;_.each(f,function(a,b){g[b]!==a&&(i=!1)});b=i}}b&&window.history.back();d.courseKey&&d.courseKey!==
w&&window.location.reload();h&&J.trackVirtualPageView();if((null===a.course||a.course.key!==d.courseKey)&&"none"!==d.courseKey)E(),c.addEventListener("courseChange",function(b){d.lessonKey||(b.lastLessonVisited?a.goToLesson(b.lastLessonVisited.key):a.goToLesson(b.lessons[0].key))},{removeAfterFired:!0}),c.changeCourse(d.courseKey);if((null===a.lesson||a.lesson.key!==d.lessonKey)&&d.lessonKey)E(),c.addEventListener("lessonChange",function(b){d.morselKey||(b.lastMorselVisited?a.goToMorsel(b.lastMorselVisited):
...
b.key&&(c+=" is-current");return c};a.getPlayerPartialForCurrentMorsel=function(){return C(a.morsel,"_player.html")};a.getPlayerExtensionPartialForCurrentMorsel=function(){return C(a.morsel,"_extension.html",!0)};a.getSidePanelPartialForCurrentMorsel=function(){return C(a.morsel,"_side_panel.html")};a.goToMorsel=function(e){(!e||!e.key)&&console.error("Cannot go to invalid morsel "+e);(!a.morsel||e.key!==a.morsel.key)&&f(function(){if("Exercise"===e.model){var a=e.lecture_ref||e.quiz_ref;c.announcePendingMorselChange(a);
b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.exercise+e.key+"/"+m.morsel+a.key)}else c.announcePendingMorselChange(e),e.containingExercise?b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.exercise+e.containingExercise.key+"/"+m.morsel+e.key):b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.morsel+e.key)})};a.goToTidbit=function(c){!c&&a.tidbit?d.exerciseKey?b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.exercise+d.exerciseKey+"/"+m.morsel+
d.morselKey):b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.morsel+d.morselKey):(!a.tidbit||c.key!==a.tidbit.key)&&f(function(){b.path("/"+m.course+d.courseKey+"/"+m.lesson+d.lessonKey+"/"+m.exercise+c.containingExercise.key+"/"+m.morsel+c.containingExercise.quiz_ref.key+"/"+m.tidbit+c.key)})};a.goToNextMorsel=function(){var b=a.morsel,c=a.morsels.length;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;b<c-1?a.goToMorsel(a.morsels[b+1]):(h.pauseVideo(),a.course?t.userIsLoggedIn()?
a.lessons&&a.lesson.key===a.lessons[a.lessons.length-1].key?(a.enrollment=_.find(t.get("_enrollments"),function(b){return b.node_key===a.course.key}),v.activate("endOfCourse",{courseIsComplete:a.course.catalog_entry.complete,courseKey:a.course.key,isPaidEnrolled:u.isCurrentEnrolled(a.enrollment,a.course),isGraduated:u.isGraduated(a.enrollment,a.course)})):v.activate("endOfLesson",{lessonKey:a.lesson.key,userIsLoggedIn:t.userIsLoggedIn()}):v.activate("endOfLessonSignInGate",{lessonKey:a.lesson.key,
userIsLoggedIn:t.userIsLoggedIn()}):v.activate("endOfIsolatedContent"))};a.goToPreviousMorsel=function(){var b=a.morsel;b.containingExercise&&(b=b.containingExercise);b=b.indexInLesson;0<b?a.goToMorsel(a.morsels[b-1]):console.error("Can't go to previous; at beginning of lesson")};a.goToLesson=function(e){if(!a.lesson||e!==a.lesson.key)c.announcePendingLessonChange(),f(function(){b.path("/"+m.course+d.courseKey+"/"+m.lesson+e)})};a.goToNextLesson=function(){if(a.lessons){var b=a.lessons.length,c=_.find(a.lessons,
function(b){return b.key===a.lesson.key}).indexInCourse;0<=c&&c<b-1?a.goToLesson(a.lessons[c+1].key):console.error("Can't go to next; at end of course!")}else console.error("Can't go to next lesson, we're not in a course!")};a.os=r.os(!1);var L=K.lessonIsCareerProject;a.shouldShowLesson=function(b){if(b=L(b))b=a.enrollment?(b=a.enrollment.product_variant)?"lite"===b.toLowerCase():!1:!1;return!b};a.isActiveMorsel=function(b){return a.morsel&&a.morsel.key===b};a.getProgressPercentage=function(){return 77};
var B}]);angular.module("viewer").controller("viewerPlayer",["$scope","lesson","$timeout","$window","currentUser","userTrials",function(a,c,b,d,e,f){a.isPlayerLoading=!0;d=function(a){var b=["attempt-1","attempt-2","control"],a=_.reduce(("skip-button-test-"+a).split(""),function(a,b){return(37*a+b.charCodeAt(0))%1009},0);return b[a%b.length]};a.skipButtonTrial=e.userIsLoggedIn()?f.getOrEnrollManually("skip_button_trial",d,[e.get("key")]):"excluded";a.loadingComplete=function(c){c?b(function(){a.$apply(function(){a.isPlayerLoading=
